...
<div class="rounded-2xl border border-slate-800/80 bg-slate-900/90 p-4 shadow-2xl backdrop-blur-sm">
	<div class="mb-3 flex items-center justify-between border-b border-slate-800/80 pb-2">
		<h2 class="text-xs font-black tracking-widest text-slate-400 uppercase">
			Battle Log
		</h2>
	...
	<div
		bind:this={logEl}
		class="h-44 space-y-1.5 overflow-y-auto font-mono text-xs pr-2 scrollbar-thin scrollbar-track-transparent scrollbar-thumb-slate-800"
	>
		{#if entries.length === 0}
