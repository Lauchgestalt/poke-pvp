...
<div class="w-80 shrink-0 rounded-2xl border border-slate-800/80 bg-slate-900/90 p-4 shadow-2xl backdrop-blur-sm">
	...
	{:else}
		<div class="space-y-2">
			{#each party as mon (mon.partySlot)}
				...
				{@const hpBarClass = pct <= 20 ? 'bg-red-500' : pct <= 50 ? 'bg-amber-400' : 'bg-emerald-500'}

				<button
					class="group relative flex w-full cursor-pointer items-center gap-3 rounded-xl border p-2.5 text-left transition-all duration-150 disabled:cursor-not-allowed disabled:opacity-50 {
						isActive
				...
				>
					...
					<div class="min-w-0 flex-1">
						<div class="flex items-center justify-between gap-1.5">
							...
							{#if isActive}
								<span class="shrink-0 rounded bg-emerald-500/20 px-1.5 py-0.5 font-mono text-[9px] font-black text-emerald-400 tracking-wider border border-emerald-500/30">
									ACTIVE
								</span>
							{:else if fainted}
								<span class="shrink-0 rounded bg-red-950/60 px-1.5 py-0.5 font-mono text-[9px] font-bold text-red-400 tracking-wider border border-red-800/40">
									FAINTED
								</span>
							{/if}
						</div>

						<div class="mt-1.5 h-2 w-full overflow-hidden rounded-full bg-slate-950 p-0.5 border border-slate-800/80">
							<div
						...
						<div class="mt-1 flex items-center justify-between font-mono text-[10px] text-slate-400">
							<span>HP</span>
